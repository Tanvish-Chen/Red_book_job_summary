...
> 最后更新：2026-09-15 · 数据规模：388 条笔记（招聘 41 / 面经 173 / 攻略 136 / 无关 38 / 待分类 0）+ 181 条评论
...
| 配置 | 当前值 | 说明 |
|---|---|---|
| `KEYWORDS` | 招聘向 8 词 | 覆盖 AI 秋招、大模型校招、27届秋招、算法内推，以及央国企、银行、研究院、外企 AI 招聘 |
...
- 实测：8 词 × 2 页约 20 分钟，一轮新增约 100 条（关键词间有重复，属正常）
- 数据落盘到 `data/xhs/jsonl/search_contents_<日期>.jsonl`

**每轮采集后检查日志**：确认 8 个关键词都跑完、无 `461`/`请通过验证`/ERROR；出现 461 = 账号进惩罚期（§6）。
...
5. 复核完删掉该条目的 `"auto": true` 标记；完全无法判断的条目把 `category` 改成 `"pending"`（看板显示为"待分类"）。**不要直接删除条目**——`auto_extract` 是增量脚本，发现 note_id 不在库里会重新生成规则草稿，把你的判断回滚（实测教训）
...
6. **夜间多轮实验实测结论（2026-09-01）**：当晚尝试 6 轮循环（90 分钟间隔、招聘/面试词交替、失效即停保护），第 1 轮第 7 分钟即被搜索 API 验证码拦截（CAPTCHA, Verifytype 216），保护机制触发终止。诱因：当天已有两轮采集、轮间隔仅 57 分钟，深夜连续搜索是异常行为模式。**结论：多轮循环只在白天分时段跑；夜间挂机一律不做**
...
| 招聘帖截止时间覆盖率低（3/52） | 多数帖子本就不写截止（数据源特性），靠"未注明"筛选兜底 |
